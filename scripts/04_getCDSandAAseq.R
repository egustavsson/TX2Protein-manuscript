...
get_sequences <- function(gtf){
  ...
  for (i in 1:NROW(NT_seq)) {
    ORF_range <- ORFik::findORFs(
      as.character(NT_seq$NT_seq[i]), longestORF = TRUE, startCodon = "ATG") %>%
      unlist() %>%
      data.frame()
    ...
    # Check if ORF_range is NA and set subsequent outputs to NA
    if (is.na(ORF_range)) {
      NT_seq$CDS_seq[i] <- NA
      NT_seq$AA_seq[i] <- NA
    } else {
      # Get CDS from coordinates
      NT_seq$CDS_seq[i] <-
        substr(
          as.character(NT_seq$NT_seq[i]), start = ORF_range$start, stop = ORF_range$end)
